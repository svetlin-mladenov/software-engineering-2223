...
class Board:
    ...
    def print_board(self):
        for i in range(8):
            for j in range(8):
                if (i + j) % 2 != 0:
                    cprint(self.all_pieces[i][j] + " ", 'red', 'on_white', end='')
                else:
                    cprint(self.all_pieces[i][j] + " ", 'white', 'on_red', end='')
            print()

print("Board in the beggining:")
board = Board('rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR')
board.print_board()
print("Board with fen string:")
board = Board("r5rk/1p1q1p1p/p7/3N4/3PnB1b/2P2Q1P/PP3P2/R3R2K b - - 0 25")
board.print_board()