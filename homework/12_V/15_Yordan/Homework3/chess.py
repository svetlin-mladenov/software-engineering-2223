...
class Chess:
    ...
    def movePiece(self, fromPosition, toPosition):
        fromCoordinates = self.getPositionCoordinates(fromPosition)
        toCoordinates = self.getPositionCoordinates(toPosition)
        piece = self.board[fromCoordinates[0]][fromCoordinates[1]]
        self.board[fromCoordinates[0]][fromCoordinates[1]] = None
        self.board[toCoordinates[0]][toCoordinates[1]] = piece

    def translatePieceFromFenFormat(self, letter):
        if letter == 'P':
        ...
        for i, row in enumerate(fen[0].split('/')):
            ...
            for letter in row:
                if letter.isdigit():
                    ...
                    self.board[j][7 - i] = self.translatePieceFromFenFormat(letter)
                    j += 1
